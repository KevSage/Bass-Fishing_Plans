...
def enrich_member_plan(plan: Dict[str, Any], weather: Dict[str, Any], phase: str) -> Dict[str, Any]:
    """
    Add gear, strategy, and pattern_summary to a member plan (with primary + secondary).
    Modifies plan in place and returns it.
    
    Args:
        plan: The LLM-generated plan
        weather: Weather data dict
        phase: Bass phase (winter, spawn, etc.)
    """
    
    # Add gear, strategy, and pattern_summary to primary
    if "primary" in plan and "base_lure" in plan["primary"]:
        ...
        plan["primary"]["strategy"] = build_strategy_for_presentation(
            plan["primary"]["presentation"],
            weather
        )
        plan["primary"]["pattern_summary"] = build_pattern_summary(
            plan["primary"]["presentation"],
            phase,
            weather
        )
    
    # Add gear, strategy, and pattern_summary to secondary
    if "secondary" in plan and "base_lure" in plan["secondary"]:
        ...
        plan["secondary"]["strategy"] = build_strategy_for_presentation(
            plan["secondary"]["presentation"],
            weather
        )
        plan["secondary"]["pattern_summary"] = build_pattern_summary(
            plan["secondary"]["presentation"],
            phase,
            weather
        )
